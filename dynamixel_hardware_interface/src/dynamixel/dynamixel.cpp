...
void Dynamixel::RWDataReset()
{
  read_data_list_.clear();
  write_data_list_.clear();
  last_sync_write_values_.clear();
  use_individual_read_fallback_ = false;
}
...
{
  if (use_individual_read_fallback_) {
    return ReadDxlDataIndividually();
  }

  // FastSyncRead is an optimization and is not supported reliably by every
  // Dynamixel/firmware/U2D2 combination.  In particular, it may work during
  // activation and then start returning a timeout/corrupt packet after torque
  // is enabled.  Never let that transient protocol failure take down the
  // ros2_control hardware: fall back to normal SyncRead immediately.
    ...
    (fast_read_permanent_ || fast_read_fail_count_ < 10))
  {
    ...
    if (comm_result == DxlError::OK) {
      ...
      fprintf(
        stderr,
        "FastSyncRead failed (attempt %d, error %d); switching to normal SyncRead.\n",
        fast_read_fail_count_, comm_result);

      use_fast_read_protocol_ = false;
      fast_read_permanent_ = false;
      fast_read_fail_count_ = 0;
  ...
  if (comm_result != DxlError::OK) {
    // Some USB/half-duplex setups occasionally return a corrupt packet for a
    // broadcast sync-read with two XM430s.  Fall back to two ordinary direct
    // reads; this is slower but robust and still fits the 50 Hz controller
    // cycle for the small position/velocity/current block.
    if (ReadDxlDataIndividually() == DxlError::OK) {
      use_individual_read_fallback_ = true;
      fprintf(
        stderr,
        "SyncRead failed; switching to individual direct reads for feedback\n");
      return DxlError::OK;
    }
    return comm_result;
  }
...
{
  for (const auto & it_read_data : read_data_list_) {
    if (it_read_data.item_addr.empty() || it_read_data.item_size.empty()) {
      continue;
    }

    uint16_t start_addr = it_read_data.item_addr.front();
    uint16_t end_addr = static_cast<uint16_t>(
      it_read_data.item_addr.front() + it_read_data.item_size.front());
    for (size_t i = 1; i < it_read_data.item_addr.size(); ++i) {
      start_addr = std::min(start_addr, it_read_data.item_addr[i]);
      end_addr = std::max(
        end_addr,
        static_cast<uint16_t>(it_read_data.item_addr[i] + it_read_data.item_size[i]));
    }

    const uint16_t read_size = static_cast<uint16_t>(end_addr - start_addr);
    std::vector<uint8_t> read_buffer(read_size, 0);
    int result = COMM_TX_FAIL;
    uint8_t dxl_error = 0;
    constexpr int MAX_INDIVIDUAL_READ_ATTEMPTS = 3;
    for (int attempt = 0; attempt < MAX_INDIVIDUAL_READ_ATTEMPTS; ++attempt) {
      std::fill(read_buffer.begin(), read_buffer.end(), 0);
      dxl_error = 0;
      result = packet_handler_->readTxRx(
        port_handler_, it_read_data.comm_id, start_addr, read_size,
        read_buffer.data(), &dxl_error);
      if (result == COMM_SUCCESS && dxl_error == 0) {
        break;
      }
      if (attempt + 1 < MAX_INDIVIDUAL_READ_ATTEMPTS) {
    ...
    if (result != COMM_SUCCESS) {
      fprintf(
        stderr,
        "Individual Read ID:%03d failed after %d attempts: %s (%d)\n",
        it_read_data.comm_id, MAX_INDIVIDUAL_READ_ATTEMPTS,
        packet_handler_->getTxRxResult(result), result);
      return DxlError::SYNC_READ_FAIL;
    }
    if (dxl_error != 0) {
      fprintf(
        stderr,
        "Individual Read ID:%03d returned device error after %d attempts: %s\n",
        it_read_data.comm_id, MAX_INDIVIDUAL_READ_ATTEMPTS,
        packet_handler_->getRxPacketError(dxl_error));
      return DxlError::SYNC_READ_FAIL;
    }
    ...
    ProcessDirectReadData(
      ...
      it_read_data.item_data_ptr_vec,
      [&read_buffer, start_addr](uint8_t, uint16_t addr, uint8_t size) {
        const size_t offset = static_cast<size_t>(addr - start_addr);
        switch (size) {
          case 1:
            return static_cast<uint32_t>(read_buffer[offset]);
          case 2:
            return static_cast<uint32_t>(DXL_MAKEWORD(
                     read_buffer[offset], read_buffer[offset + 1]));
          case 4:
            return static_cast<uint32_t>(DXL_MAKEDWORD(
                     DXL_MAKEWORD(read_buffer[offset], read_buffer[offset + 1]),
                     DXL_MAKEWORD(read_buffer[offset + 2], read_buffer[offset + 3])));
          default:
...
DxlError Dynamixel::ProcessReadCommunication(
  ...
  [[maybe_unused]] double period_ms,
  bool is_sync,
  bool is_fast)
{
  // txPacket() in the SDK installs a timeout based on the expected reply
  // size.  If a corrupt status packet is received, discard any remaining
  // bytes and retry the complete request once.  This prevents one bad USB
  // packet from freezing feedback for both joints.
  constexpr int MAX_READ_ATTEMPTS = 2;
  for (int attempt = 0; attempt < MAX_READ_ATTEMPTS; ++attempt) {
    ...
    const char * read_name = is_sync ?
      (is_fast ? "FastSyncRead" : "SyncRead") :
      (is_fast ? "FastBulkRead" : "BulkRead");
    if (attempt + 1 < MAX_READ_ATTEMPTS &&
      (dxl_comm_result == COMM_RX_TIMEOUT || dxl_comm_result == COMM_RX_CORRUPT))
    {
      fprintf(
        stderr, "%s Rx Fail [Dxl Size : %ld] [Error code : %d], "
        "clearing RX buffer and retrying\n",
        read_name, read_data_list_.size(), dxl_comm_result);
    ...
    fprintf(
      stderr, "%s Rx Fail [Dxl Size : %ld] [Error code : %d]\n",
      read_name, read_data_list_.size(), dxl_comm_result);
    return is_sync ? DxlError::SYNC_READ_FAIL : DxlError::BULK_READ_FAIL;
  }
...
{
  std::vector<std::pair<uint8_t, std::vector<uint8_t>>> pending_values;
  ...
  for (auto it_write_data : write_data_list_) {
    ...
    const auto previous = last_sync_write_values_.find(comm_id);
    if (previous == last_sync_write_values_.end() || previous->second != param_write_value) {
      has_changed = true;
    }
    pending_values.emplace_back(comm_id, std::move(param_write_value));
  }

  // A position-mode Dynamixel keeps its goal while torque is enabled.  A
  // no-op write only consumes bus time and can interfere with the next read.
  if (!has_changed) {
    return DxlError::OK;
  }

  for (auto & pending : pending_values) {
    if (group_sync_write_->addParam(pending.first, pending.second.data()) != true) {
  ...
  if (dxl_comm_result != COMM_SUCCESS) {
    ...
    for (auto & pending : pending_values) {
      last_sync_write_values_[pending.first] = std::move(pending.second);
    }
    return DxlError::OK;
  }
}
