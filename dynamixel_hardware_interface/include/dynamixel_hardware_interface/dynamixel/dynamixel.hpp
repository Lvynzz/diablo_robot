...
private:
  ...
  // Fast Sync/Bulk Read is not reliable with the XM430 firmware used by the
  // Diablo joints.  Normal Sync/Bulk Read is supported and has a complete
  // per-packet timeout calculation in the SDK.
  bool use_fast_read_protocol_ = false;
  bool fast_read_permanent_ = false;
  int fast_read_fail_count_ = 0;
  // Fallback for a bus where a multi-ID sync-read packet is corrupted.  Each
  // ID is then read with one contiguous direct READ request.
  bool use_individual_read_fallback_ = false;
  ...
  // Position-mode Dynamixels hold their last goal internally.  Avoid sending
  // an identical sync-write packet on every ros2_control cycle.
  std::map<uint8_t /*comm_id*/, std::vector<uint8_t>> last_sync_write_values_;
