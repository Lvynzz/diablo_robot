...
int main(int argc, char **argv)
{
    ...
    const auto controller_port = node->declare_parameter<std::string>(
        "controller_port", "/dev/diablo_controller");
    RCLCPP_INFO(node->get_logger(), "Using Diablo controller serial port: %s",
                controller_port.c_str());
    if(Hal.init(controller_port)) return -1;
